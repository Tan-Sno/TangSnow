...
def banner(zh_title: str, en_title: str, version: str, updated: str) -> str:
    # 元信息中英对照：这两份文件对外发布，标题与目录本就双语，若注释与引用块只有中文，
    # 英文读者第一眼看到的仍是中文 —— 与「仓库默认英语」的定位不一致。
    return (
        "<!-- ⚠️ 本文件是自动导出产物，请勿直接编辑。\n"
        ...
        "> This file matches the legal text shown in the app under "
        "\u201cSettings \u2192 About TangSnow\u201d, and corresponds to "
        "`POLICY_VERSION = %s` in the app.\n"
        "> 最后更新 / Last updated：%s\n"
        "> **请勿直接编辑**；请修改 `strings.xml` 后运行 `python tools/export_legal_docs.py` 重新导出。\n"
        "> **Do not edit directly** \u2014 change `strings.xml` and re-run "
        "`python tools/export_legal_docs.py`.\n\n"
        "**目录 / Contents**\u3000[English](#english) \u00b7 [中文](#%s)\n\n"
        "---\n\n" % (en_title, zh_title, version, version, updated, zh_title)
    )
...
def main() -> int:
    ...
    for key, (rel, zh_title, en_title) in DOCS.items():
        ...
        # 英文段在前：仓库对外文档以英文为默认语言（README 等亦然），法律文本是同一批
        # 对外文本，若中文段在前，英文读者要滚过整段中文才看到自己的语言。
        # 两段仍**同页对照**（这是刻意保留的：任何语言的读者都不会漏看另一种）。
        content += "## English\n\n%s\n\n---\n\n## %s\n\n%s\n" % (en_body, zh_title, zh_body)
        out = ROOT / rel
